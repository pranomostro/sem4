...
class Production:
    ...
    def apply(self, w):
        r = []
        for i in range(0, len(w) - len(self.lhs) + 1):
            if self.lhs == w[i:i + len(self.lhs)]:
                r.append(w[0:i] + self.rhs + w[i+len(self.lhs):])
        return r
...
class Context_Sensitive_Grammar:
    ...
    def enumerate_words_raw(self, max_len):
        words = set()
        worklist = Queue()
        words.add(self.start_symbol)
        worklist.put(self.start_symbol)

        while not worklist.empty():
            w = worklist.get()
            for p in self.productions:
                new = [u for u in p.apply(w) if len(
                    u) <= max_len and not u in words]
                words.update(set(new))
                for u in new:
                    worklist.put(u)
    ...
    def enumerate_words(self, max_len):
        def is_terminal(w):
            return w.islower() or len(w) == 0

        return set(w for w in self.enumerate_words_raw(max_len) if is_terminal(w))
