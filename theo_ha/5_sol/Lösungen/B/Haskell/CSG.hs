...
apply (lhs :->: rhs) [] = if null lhs then [rhs] else []
apply p@(lhs :->: rhs) (x : xs) = map (x:) (apply p xs) ++
  case stripPrefix lhs (x : xs) of
    Nothing  -> []
    Just xs' -> [rhs ++ xs']

enumerateWordsRaw :: CSG -> Int -> Set [Symbol]
enumerateWordsRaw g l = go S.empty (S.singleton (startWord g) `S.union`
                          (if hasEpsilon then S.singleton [] else S.empty))
  where hasEpsilon = trivialProduction g `S.member` productions g
        prods      = S.toList $ S.delete (trivialProduction g) (productions g)
        go s workSet =
          case S.minView workSet of
            Nothing -> s
            Just (w, workSet) -> let newWords = [w' | p <- prods, w' <- apply p w, length w' <= l, not (w' `S.member` s)]
                                 in  go (S.insert w s) (foldr S.insert workSet newWords)
        
enumerateWords :: CSG -> Int -> Set String
enumerateWords g l = S.fromList $ mapMaybe toWord $ S.toList $ enumerateWordsRaw g l
  where toWord :: [Symbol] -> Maybe String
        toWord = mapM (\x -> case x of {Terminal c -> Just c; _ -> Nothing})
